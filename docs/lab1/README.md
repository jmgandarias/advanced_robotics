...
- **`cartesian_planning`**: Script that performs the complete simulation during all proposed segments, using the Robotic Toolbox with the ABB IRB120 manipulator model and the graphical representation of the temporal evolution of Cartesian trajectories and orientation in $ZYZ$ Euler angles.
    ...
    2. Here you call the function ```generate_smooth_path``` that you have to code in [Exercise 1](#1-quaternion-interpolation)
...
Figure 1 shows the described situation, using the example of concatenating a displacement from location $P_1$ to $P_2$ with another from $P_2$ to $P_3$. To avoid the velocity discontinuity that occurs at $P_2$, a constant acceleration is used to adapt the velocity variation of vector $X$ from the first segment to the second.

<img src="images/smooth_trajectory.png" alt="smooth_trajectory" width="400"/>

*Figure 1. Diagram of the variation of position and velocity in the movement from $P_1$ to $P_3$ via $P_2$.*

This way, $-\tau$ units of time before reaching $P_2$ (time 0), the velocity will be linearly changed from $\Delta P_2/T_1$ to $\Delta P_3/T_2$, to accommodate the velocity $\tau$ units of time after passing $P_2$. Thus, the problem is defined as the calculation of a quadratic function $X(t)$ that starts at point $P_A$ and ends at $P_B$ (start and end points of the smoothing) defined in the time range $[-\tau, \tau]$.
...
<a id="equation-1"></a>
$$
P(t) = P_2 - \frac{(\tau - t)^2}{4\tau T_1} \Delta P_2 + \frac{(\tau + t)^2}{4\tau T_2} \Delta P_3
$$
...
Considering all the above, and the following values for $P_1, P_2, P_3$, the following exercises are requested:
...
Create a MATLAB function in the format `P=generate_smooth_path(P0, P1, P2, tau, T, t)` that calculates the transformation $P$ corresponding to the movement from $P_1$ to $P_3$ via $P_2$ smoothed by the Taylor method. The parameters $\tau$ and $T$ correspond respectively to the transition interval and total time used to traverse the path as shown in Figure 1, and $T$ indicates the time at which the location of the calculated path $P$ is reached.
